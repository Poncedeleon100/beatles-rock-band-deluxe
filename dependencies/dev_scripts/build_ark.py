...
def build_patch_ark(xbox: bool, rpcs3_directory: str = None, rpcs3_mode: bool = False):
    ...
    print("Building The Beatles: Rock Band Deluxe patch arks...")
    ...
    if rpcs3_mode:
        if platform == "win32":
            build_location = rpcs3_directory + "\\game\\BLUS30282\\USRDIR\\gen"
        else:
            build_location = rpcs3_directory + "/game/BLUS30282/USRDIR/gen"
    else:
        if platform == "win32":
    ...
    # pull the latest changes from the The Beatles: Rock Band Deluxe repo if necessary
    # if not check_git_updated(repo_url="https://github.com/hmxmilohax/beatles-rock-band-deluxe", repo_root_path=root_dir):
        # cmd_pull = "git pull https://github.com/hmxmilohax/beatles-rock-band-deluxe main".split()
    ...
    if not failed:
        print("Successfully built The Beatles: Rock Band Deluxe ARK.")
        return True
    else:
